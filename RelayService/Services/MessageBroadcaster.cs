...
public class MessageBroadcaster : IMessageBroadcaster
{
    ...
    public async Task SendChatMessageToUserAsync(string username, ChatMessage chatMessage)
    {
        // Serialize chat message to JSON
        var json = JsonSerializer.Serialize(chatMessage);
        var bytes = Encoding.UTF8.GetBytes(json);

        // Delegate sending to the connection manager
        await connectionManager.SendMessageToUserAsync(username, bytes);

        Console.WriteLine($"Sent private message from {chatMessage.FromUsername} to user {username}");
    }
    ...
    /// <param name="chatMessage">The chat message object to serialize and broadcast</param>
    /// <returns>A task representing the asynchronous broadcast operation</returns>
    public async Task BroadcastChatMessageToServerAsync(string serverId, ChatMessage chatMessage)
    {
        // Serialize chat message to JSON
        var json = JsonSerializer.Serialize(chatMessage);
        var bytes = Encoding.UTF8.GetBytes(json);
        ...
        // Delegate broadcasting to the connection manager
        await connectionManager.BroadcastMessageToServerAsync(serverId, bytes);

        Console.WriteLine($"Broadcasted server message from {chatMessage.FromUsername} to server {serverId}");
    }
}
